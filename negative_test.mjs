...
const MUTATIONS = [
    ...
    ['the status line reports a doomed prefill as applied', 'index.js',
        '    const atRisk = Boolean(lastReport.applied && lastReport.detail?.mergeRisk);',
        '    const atRisk = false;', 'load_test.mjs'],
    ['a reason code ships with no display text', 'index.js',
    ...
    ['the premerge stops folding the name', 'engine.js',
        ...
        '            const targetContent = typeof target.content === \'string\' ? target.content : null;\n            const previousContent = typeof previous.content === \'string\' ? previous.content : null;', 'wire_test.mjs'],
    ...
    ['the skip path is allowed to mutate before returning', 'engine.js',
        ...
        '    if (false) {\n        return { applied: false, reason: REASON.NOTHING_TO_DO, detail };\n    }',
        'wire_test.mjs'],
    ['an unsurvivable prefill is reported as a clean success', 'engine.js',
        '    if (targetWillBeMergedAway(data, messages, index)) {', '    if (false) {', 'fuzz_test.mjs'],
    ['merge risk goes back to being a property of the flag', 'engine.js',
        '    if (targetWillBeMergedAway(data, messages, index)) {',
        '    if (flag.name && targetWillBeMergedAway(data, messages, index)) {', 'test.mjs'],
    ['merge risk ignores a non-merging server', 'engine.js',
        '    if (!serverWillMerge(data)) {\n        return false;\n    }',
        '    if (false) {\n        return false;\n    }', 'test.mjs'],
    ['merge risk treats unpredictable content as safe', 'engine.js',
        '    if (targetContent === null || previousContent === null) {\n        return true;\n    }',
        '    if (targetContent === null || previousContent === null) {\n        return false;\n    }', 'test.mjs'],
    ['merge risk forgets that an empty message survives', 'engine.js',
        '    return targetContent.length > 0;', '    return true;', 'test.mjs'],
    ['merge risk ignores the predecessor role', 'engine.js',
        "    if (target?.role !== 'assistant' || previous?.role !== 'assistant') {",
        '    if (false) {', 'test.mjs'],
    ['a doomed seed still flips the thinking channel', 'engine.js',
        "    if (cfg.ensureThinking && detail.reasoningField && !detail.mergeRisk && !data.include_reasoning) {",
        "    if (cfg.ensureThinking && detail.reasoningField && !data.include_reasoning) {", 'test.mjs'],
